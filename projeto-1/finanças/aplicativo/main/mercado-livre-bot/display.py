...
def exibir_cabecalho():
    console.print()
    console.print(Panel(
        Text("PROMOCOES DO MERCADO LIVRE", style="bold white", justify="center"),
...
def exibir_estatisticas():
    ...
    console.print(Panel(table, title="[bold green]ESTATISTICAS[/]", border_style="green"))
    console.print()
...
def exibir_produto(produto, index=None):
    ...
    header += titulo[:60] + ("..." if len(titulo) > 60 else "")
    ...
    if specs:
        lines.append(f"[bold blue]Specs:[/] {specs}")
...
def exibir_lista_produtos(produtos, titulo="PRODUCOS ENCONTRADOS"):
    console.print()
    console.print(f"[bold {titulo}[/]", style="bold white")
    console.print("-" * 80)
...
def exibir_titulo_secao(titulo):
    console.print()
    console.rule(f"[bold white]{titulo}[/]", style="cyan")
    console.print()
