    ...
    try:
        cursor.execute("""
            INSERT INTO produtos (
                titulo, url_original, url_afiliado, preco_original,
                preco_desconto, porcentagem_desconto, imagem_url,
                especificacoes, categoria, data_coleta
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            produto["titulo"],
        ...
        if produto.get("preco_desconto"):
            cursor.execute("""
                ...
                VALUES (?, ?)
            """, (produto_id, produto["preco_desconto"]))
