...
from config import UPDATE_INTERVAL, ML_AFFILIATE_ID
from database import init_db, inserir_produtos_em_lote, obter_estatisticas, buscar_todas_promocoes
...
def carregar_produtos_do_banco():
    global produtos
    produtos = buscar_todas_promocoes(limite=100)
    return produtos
...
def executar_scraping():
    ...
    if not ML_AFFILIATE_ID:
        ...
        console.print("[dim]Configure a variavel ML_AFFILIATE_ID no arquivo .env[/]")
        console.print()

    exibir_titulo_secao("INICIANDO SCRAPING")
...
def main():
    ...
    exibir_cabecalho()
    console.print("[bold cyan]Bot de Promocoes do Mercado Livre[/]")
